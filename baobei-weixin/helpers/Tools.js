class Tools{
	 ...
	 */
	 getToday() {
	 	return this.formatTime(new Date()).slice(0,10)
	 }
	 ...
	 */
	 formValidate(validateList) {
		...
		for (let i = 0; i < validateList.length; i++) {
		    ...
		    // 判断空与非空
		    if (item.empty && item.val.length <= 0) {
		        // 显示消息
		        curPage.wetoast.toast({
		            title: item.empty.tips
		        })
		        result = false
		        break
		    }
		    ...
		    // 判断最小长度
		    if (item.minlength && item.val.length < item.minlength.val) {
		        // 显示消息
		        curPage.wetoast.toast({
		            title: item.empty.tips
		        })
		    ...
		    // 判断正则
		    if (item.reg && !item.reg.val.test(item.val)) {
		        curPage.wetoast.toast({
		            title: item.reg.tips
		        })
		        result = false
